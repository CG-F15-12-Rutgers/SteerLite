...
namespace SteerLib
{
	...
	*/
	...
	class STEERLIB_API AStarPlannerNode;
	typedef std::shared_ptr<AStarPlannerNode> AStarNode;
	class AStarPlannerNode {
	public:
		AStarPlannerNode(unsigned int index, double g, double h) : _index(index), _g(g), _f(h), _parent(nullptr) {}
		
		double index() const { return _index; }
		double g() const { return _g; }
		double f() const { return _f; }
		AStarNode parent() { return this->_parent; }

		void g(double val) { _g = val;}
		void f(double val) { _f = val;}
		void parent(AStarNode node) { this->_parent = node;}
	...
	public:
		...
		*/
		bool canBeTraversed(int id, Util::Point current, std::vector<AStarNode>& neighborset);
		/*
	...
	private:
		void pop_min(std::vector<AStarNode>& list, AStarNode& current_astar);
		double dist_between(Util::Point point1, Util::Point point2);
		SteerLib::GridDatabase2D * gSpatialDatabase;
		void renewF(AStarNode& node_astar, Util::Point goal);
